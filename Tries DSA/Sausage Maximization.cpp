...
void insert(ll x){
	...
	for(ll i=42;i>=0;i--){
		ll b=(x>>i)&1;
		if(cur->child[b]==NULL) cur->child[b]=new Trie();
		cur=cur->child[b];
		cur->starts++;
	}
	cur->isEnd=true;
...
ll query1(ll x){
	...
	for(ll i=42;i>=0;i--){
		...
		if(cur->child[b^1] and cur->child[b^1]->starts>0){
...
void remove(ll x){
	...
	if(cur->starts<=0) cur->isEnd=false;
}
...
void madara(){
	...
	insert(0);
	// you taken prefx from vs and remove that part and search m,ax suffix xor
	ll x1=0;
	ll ans=0;
	for(ll i=0;i<vs.size();i++){
		x1^=vs[i];
		remove(suf[i]);
		ans=max(ans,query1(x1));
