...
ll query(ll p,ll l){
  ...
  for(ll i=28;i>=0;i--){
    ...
    if(lbit){
      // prefer 0
      if(cur->child[pbit] and cur->child[pbit]->starts>0){
        ans+=cur->child[pbit]->starts;
      }
      cur=cur->child[pbit^1];

    }
    else{
      // prefer same or else u violate the cond
      cur=cur->child[pbit];
    }
  }
  return ans;
...
void madara(){
	...
	loo(ii,q){
      ...
      remove(x);
			...
			ll p,l;
			cin>>p>>l;
      // on xoring with p how many res will be <l we need this
      pri(query(p,l))
