...
// pre --> current prefix
// finds how many elements if we do xor with pre from trie we would get xor<k
ll query(ll pre,ll k){
	...
	for(ll i=20;i>=0;i--){
		...
		ll pbit=(pre>>i)&1;
		if(kbit){
			// if there is some bit (NOT null) where pbit and cur bit are same => xor bit 0(at this [pos]) but in k its its 1 ==> xor<k for entire branch (starting from all numbers)
			if(cur->child[pbit]) ans+=cur->child[pbit]->starts;

			cur=cur->child[1-pbit];

		}
		else{
			// only target same bit if different bit than its always >k
			cur=cur->child[pbit];
		}
	}
	return ans;
