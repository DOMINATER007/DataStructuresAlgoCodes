...
void insert(string &s){
	Trie* cur=root;
	for(auto it:s){
		if(cur->child[it-'0']==NULL){
			cur->child[it-'0']=new Trie();
		}
		cur=cur->child[it-'0'];
		cur->starts++;
	}
...
ll query(string &s){
	...
	for(auto it:s){
		loo(i,10){
			if(p[it-'0']>p[i] and cur->child[i]){
				ans+=cur->child[i]->starts;
			}
		}
		if(cur->child[it-'0']) cur=cur->child[it-'0'];
		else break;
	}
	return ans;
}
