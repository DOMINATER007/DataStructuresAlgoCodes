...
void insert(string &s){
	...
	for(auto it:s){
		if(cur->child[it-'a']==NULL){
			cur->child[it-'a']=new Trie();
		}
		cur=cur->child[it-'a'];
		if(mp[s]>cur->frq or (mp[s]==cur->frq and s<cur->bst)){
			cur->frq=mp[s];
			cur->bst=s;
		}
